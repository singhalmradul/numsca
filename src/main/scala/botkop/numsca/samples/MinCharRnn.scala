...
object MinCharRnn {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /** inputs,targets are both list of integers. hprev is Hx1 array of initial
      * hidden state returns the loss, gradients on model parameters, and last
      * hidden state
      */
    def lossFun(inputs: List[Int], targets: List[Int], hprev: Tensor) = {
      ...
      var loss = 0.0
      // forward pass
      for t <- inputs.indices do {
        xs(t) = ns.zeros(vocab_size, 1) // encode in 1-of-k representation
        xs(t)(inputs(t), 0) := 1
        val it =
          if t > 0 then t
          else inputs.length // mimic python logic for negative indices
        hs(t) = ns.tanh(
          ns.dot(Wxh, xs(t)) + ns.dot(Whh, hs(it - 1) + bh)
        ) // hidden state
        ys(t) = ns.dot(
          Why,
          hs(t)
        ...
        ps(t) =
          ns.exp(ys(t)) / ns.sum(ns.exp(ys(t))) // probabilities for next chars
        loss += -ns
      ...
      for t <- inputs.indices.reverse do {
        val dy = ns.copy(ps(t))
        dy(
          targets(t),
          0
        ...
        val it =
          if t > 0 then t
          else inputs.length // mimic python logic negative indices
        dWhh += ns.dot(dhraw, hs(it - 1).T)
        dhnext := ns.dot(Whh.T, dhraw)
      }
      for dparam <- List(dWxh, dWhh, dWhy, dbh, dby) do {
    ...
    val (mWxh, mWhh, mWhy) =
      (ns.zerosLike(Wxh), ns.zerosLike(Whh), ns.zerosLike(Why))
    val (mbh, mby) =
      (ns.zerosLike(bh), ns.zerosLike(by)) // memory variables for Adagrad
    var smooth_loss =
      -math.log(1.0 / vocab_size) * seq_length // loss at iteration 0

    val hprev = ns.zeros(hidden_size, 1)

    while true do {
      // prepare inputs (we're sweeping from left to right in steps seq_length long)
      if p + seq_length + 1 >= data.length || n == 0 then {
      ...
      if n % 100 == 0 then
        println(s"iter $n, loss $smooth_loss") // print progress
      ...
      List(Wxh, Whh, Why, bh, by)
        ...
        .foreach { case (param, dparam, mem) =>
          mem += dparam * dparam
          param += -learning_rate * dparam / ns.sqrt(
            mem + 1e-8
          ) // adagrad update
        }
